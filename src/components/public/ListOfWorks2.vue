<template>
        <div class='production' v-if='isDetail'>
            <div class='search'>
            ...
            <div class='list_box'>
                <ul  v-infinite-scroll="load" infinite-scroll-disabled="disabled" class='pro_list'
                 :class="{'pro_list_3':type.showType=='works'}">
                    <div class='add_box'>
                      <!-- <img class='add_btn' src='@/assets/img/uploader/video_add.png' @click='addinfo()' > -->
                      <img class='add_btn' src='@/assets/img/allAdd.png' @click='addinfo()' >
                      <img v-if='disabledAdd' class='add_btn_disabled' src='@/assets/img/uploader/disable303.png' >
                    </div>
                    <!-- <li class='list_item' v-for="(item,index) in info.listArr" :key="index"  :class="{'forbid':item.state==-1}"> -->
                    <li class='list_item' v-for="(item,index) in info.listArr" :key="index">
                        ...
                        <div class='video_info'>
                            ...
                            <!-- 视频信息 -->
                            <ul class='ul_box' v-if='type.showType =="works"' @click='toEditPro(item)' style='cursor: pointer;'>
                                <li class='video_title'>
                                    <el-popover placement="top" width="auto" trigger="hover" :content="item.title" :disabled='item.title&&item.title.length<12?true:false'>
                                      <template #reference>
                                          <p>{{item.title}}</p>
                                      </template>
                                   </el-popover>
                                ...
                                <li  class='ellipsis' >
                                  <el-popover placement="top" width="auto" trigger="hover" :content="item.authorName" :disabled='item.authorName&&item.authorName.length<12?true:false'>
                                      <template #reference>
                                ...
                                <li  class='ellipsis' >
                                  <el-popover  v-if='item.subjectNames' placement="top" width="auto" trigger="hover" :content="item.subjectNames.join('')"
                                   :disabled='item.subjectNames.join(" ").length<12?true:false'>
                                      <template #reference>
                                          <p>专题:{{item.subjectNames.join(' ')}}</p>
                                      </template>
                                   </el-popover>
                                    <p v-else>专题:{{item.subjectNames}}</p>
                                </li>
                                <li  class='ellipsis' >
                                    <el-popover v-if='item.categoryName' placement="top" width="auto" trigger="hover" :content="item.categoryName" :disabled='item.categoryName&&item.categoryName.length<12?true:false'>
                                      <template #reference>
                                          <p>分类:{{item.categoryName}}</p>
                                      </template>
                                   </el-popover>
                                    <p v-else>分类:{{item.categoryName}}</p>
                                </li>
                                <li>播放量:{{item.timesWatched}}&nbsp;&nbsp;观看人数：{{item.viewers}}</li>
                                <li>创建时间:{{item.createdAt?item.createdAt.split('T')[0]: '' }}</li>
                                <li>最近修改时间:{{item.updatedAt?item.updatedAt.split('T')[0]: ''}}</li>
                            </ul>
                            <div class='price'  v-if='type.showType =="works"'>
                                <!-- <div class='price_item'>
                                    <p class='return_date'>{{item.recoverTime}}</p>
                                    <p class='old_price'>{{item.oldPrice}}</p>
                                </div>
...
import * as addOrEdite from '../homePage/account/addOrEdite/addOrEdite'
import * as uploaderFns from '../homePage/upMain/uploader'
...
export default {
  ...
  setup (props){
    ...
    const initVideoList=()=>{
      ...
      }else if(props.type.showType=='categoryList'){//分类列表
    ...
    function changedSearchInfo(pitchOn,type) {
         ...
         if(props.type.showType=='works'&&ids.length>0){
         ...
         }else if(props.type.showType=='categoryList'&&ids.length>0){
            if(type==='fontSearch_disable'){
           ...
           if(props.type.showType=='works'&&type!=='fontSearch'){
               ElMessage.warning({
                      message: "请先选择视频",
                      type: "warning",
               });
           }else if(props.type.showType=='categoryList'&&type!=='fontSearch'){
               ElMessage.warning({
                      message: "请先选择类别",
                      type: "warning",
               });
    ...
    const jumping = (item)=>{
      ...
      }else if(props.type.showType=='categoryList'){
    ...
    const toEditPro = (item)=>{
       router.push({path: "/homePage/account/AddAccount",query:{id:item.id}})
    }
    const addinfo = ()=>{
      if(props.type.showType=='works'){
          if(route.params.name=='uploader'){
            router.push({path: "/homePage/account/AddAccount",query:{author:route.params.id}})
          }else if(route.params.name=='classify'){
            router.push({path: "/homePage/account/AddAccount",query:{type:route.params.id}})
          }else if(route.params.name=='special'){
            router.push({path: "/homePage/account/AddAccount",query:{theme:route.params.id}})
          }
      }else if(props.type.showType=='categoryList'){
