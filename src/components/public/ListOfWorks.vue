<template>
	<div class='production1' v-if='isDetail'>
		<!-- 切换作品类型 -->
		<div class="tab_change">
			<p v-for="(item, index) in tabData" :key="index">
				<el-popover placement="bottom" width="auto" trigger="hover" :content="item.name">
					<template #reference>
						<img :src="idx == index ? item.image_act : item.image" style="width:40px;height:40px;cursor: pointer;" @click="clickImg(item, index)" />
					</template>
				</el-popover>
			</p>
		</div>
		...
		<!-- 下方列表 -->
		<div class='list_box'>
			<ul v-infinite-scroll="load" infinite-scroll-disabled="disabled" class='pro_list pro_list_3'>
				<div class='add_box'>
				...
				<li class='list_item' v-for="(item,index) in listArr" :key="index">
					...
					<div class='video_info'>
						...
						<ul class='ul_box' v-if='type.showType =="works"' @click='toEditPro(item)' style='cursor: pointer;'>
							...
							<li class='ellipsis'>
								<el-popover v-if='item.subjectNames' placement="top" width="auto" trigger="hover" :content="item.subjectNames.join('')" :disabled='item.subjectNames.join(" ").length<12?true:false'>
									<template #reference>
...
export default {
	...
	setup(props) {
		...
		let noMore = computed(
			() => info.queryWorks.total == info.listArr.length
		)
		...
		function changedSearchInfo(pitchOn, type) {
			...
			if (pitchOn === true) {
				...
				if (
					info.listArr.filter((v) => v.choosed === false).length < 1
				) {
					info.listArr = info.listArr.map((item) => {
						item.choosed = false
						return item
		...
		const jumping = (item) => {
			if (info.queryWorks.mimeType == 'POST_VIDEO') {
				router.push(`/preview/${item.id}`)
			} else if (info.queryWorks.mimeType == 'POST_IMAGE') {
				// router.push({
				// 	name: 'categoryEdit',
				// 	params: { id: item.id, name: 'POST_TEXT' },
				// })
			} else if (info.queryWorks.mimeType == 'POST_TEXT') {
				// router.push({
				// 	name: 'projectEdit',
				// 	params: { id: item.id, name: 'special' },
				// })
			} else {
			}
		}
		const toEditPro = (item) => {
			if (info.queryWorks.mimeType == 'POST_VIDEO') {
				...
				router.push({
					path: '/homePage/imgAccount/imgAccountAdd',
					query: { id: item.id },
				})
			} else if (info.queryWorks.mimeType == 'POST_TEXT') {
				// router.push({
    ...
    //添加作品
		const addinfo = () => {
          ...
          if (route.params.name == 'uploader') {
            router.push({
              path: '/homePage/account/AddAccount',
              query: { author: route.params.id },
            })
          } else if (route.params.name == 'classify') {
            router.push({
              path: '/homePage/account/AddAccount',
              query: { type: route.params.id },
            })
          } else if (route.params.name == 'special') {
            router.push({
              path: '/homePage/account/AddAccount',
              query: { theme: route.params.id },
            })
          }
			} else if (info.queryWorks.mimeType == 'POST_IMAGE') {
				if (route.params.name == 'uploader') {
		...
		function clickImg(item, index) {
			info.idx = index
			info.queryWorks.mimeType = item.mimeType
			refresh()
		}
		return {
			...toRefs(info),
