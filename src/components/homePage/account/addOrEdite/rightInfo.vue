...
import * as rightInfoFns from "./rightinfo";
export default {
  ...
  setup(props) {
    let getPObj = JSON.parse(JSON.stringify(props.pObj));
    console.log('getPObj', getPObj)
    const data = reactive({
      // 上面的图标
      headerIcon: [
        {
      ...
      choosedHeaderIcon: {
        iconUrl: require("@/assets/icon/heightPrecision/webpage_1_high.png"),
        action: props.pObj.infoTab ? props.pObj.infoTab : "INTERNET_SITE",
        title: props.pObj.infoTabTitle ? props.pObj.infoTabTitle : "网址",
        choosed: true,
        titleInputVal: props.pObj.infoIp
          ? JSON.parse(JSON.stringify(props.pObj)).infoIp
          : "", // 表示输入的查询、url、脚本
        titleInputType: props.pObj.titleInputType ? props.pObj.titleInputType : "url",
        videoPage: [
          // {
          //   comment: "", // 备注
    ...
    if (getPObj.infoTab) {
      if (getPObj.content.videoPage && getPObj.content.videoPage.length > 0) {
      ...
      if (getPObj.content.imagePage && getPObj.content.imagePage.length > 0) {
      ...
      let obj = {
        ...
        choosed: true,
        iconUrl: data.headerIcon.filter((v) => v.action === props.pObj.infoTab)[0]
          .iconUrl,
        title: data.headerIcon.filter((v) => v.action === props.pObj.infoTab)[0].title,
        titleInputType: data.headerIcon.filter((v) => v.action === props.pObj.infoTab)[0]
          .titleInputType,
        titleInputVal: "",
        imagePage:
        ...
        switch (getPObj.infoTab) {
          ...
          case "SELF_BUILT_CONTENT":
            if (getPObj.content && getPObj.content.docId) {
              rightInfoFns.textDetails({ id: getPObj.content.docId }).then((res) => {
    ...
    function save() {
      let attr = {
        id: props.pObj.infoId,
        iwpostId: props.pObj.centerObjVideo.videoId,
      ...
      rightInfoFns.infoFlowSaveContent(attr);
    }
    // --------------------------------------- 组件相关
    // 关闭的选中组件的回调
    const drawerClose = () => {
    ...
    const blurInput = () => {
      if (data.choosedHeaderIcon.action === "SELF_BUILT_CONTENT") {
        const attr = {
          authorId: props.pObj.centerObjVideo.authorId, // 作者id
          content: data.choosedHeaderIcon.docComment, // 输入的内容
          id: data.choosedHeaderIcon.docId
            ? data.choosedHeaderIcon.docId
            : getPObj.centerObjVideo.docId, // 文档id 保存后返回来的
          infoId: props.pObj.infoId, // 信息流的id
          name: "", // 当前编辑的文档的名称
        };
        rightInfoFns.textSave(attr).then((res) => {
