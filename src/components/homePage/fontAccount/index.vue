// 图片内容
<template>
	<div class='text_content' style='height:calc(100% - 42px)'>
		<div class="topSearchBox">
			<el-row :gutter="24" class="searchRow">
				<el-col :span="6">
					<Search :searchInfo="searchInfo1" @changedSearchInfo="changedSearchInfo"></Search>
				</el-col>
				<el-col :span="6">
...
export default {
	...
	setup() {
		...
		onMounted(() => {
			...
			//搜索相关下拉数据
			(async () => {
				searchParmas.searchInfo1[0].buttonArr =
					await addOrEdite.dropDownList()
				searchParmas.searchInfo1[1].buttonArr =
					await addOrEdite.authorDropDownList()
			})()
		//下拉加载
		const loading = ref(false)
		let noMore = computed(
			() => imgData.queryWorks.total == imgData.imgArr.length
		)
		...
		function changedSearchInfo(pitchOn, type, arr) {
			if (type === 'author') {
				...
				console.log(
					'🚀 ~ file: index.vue ~ line 326 ~ changedSearchInfo ~ pitchOn',
					pitchOn
				)
			...
			if (pitchOn === true) {
				...
				if (
					imgData.imgArr.filter((v) => v.choosed === false).length < 1
				) {
					imgData.imgArr = imgData.imgArr.map((item) => {
						item.choosed = false
						return item
		...
		//新增
		function add() {
			router.push({ path: '/homePage/fontAccount/FontAccountAdd' })
		}
		//预览
		const imgPreview = (id) => {
			 router.push({name: "fonImgtPreview",params:{id:id,type:'font'}})
			// router.push(`/fontAccountPreview/${id}`);
		}
		//编辑
		const imgAccountEdit = (id) => {
			router.push({
				path: '/homePage/fontAccount/FontAccountAdd',
				query: { id: id },
			})
		}
