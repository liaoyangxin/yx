	...
	<div class='font_Account_pre'>
		<div class='side_bar'>
			...
			<ul class='middle_box'>
				...
				<li>
					<img :src="parametersData.imageUrl?parametersData.imageUrl:require('@/assets/position/listImage.png')" /><br />
				</li>
				<li>
		...
		<div class='right_content' v-if='rightShowType=="img"'>
		...
		<div class='right_text_content' v-if='rightShowType=="font"'>
			<div class='text_title'>{{parametersData.title}}</div>
			<div class='text_content'>
				{{parametersData.content}}
			</div>
		</div>
		<!-- 分享模态框 -->
...
export default {
	...
	setup(props, context) {
		...
		nextTick(() => {
			data.rightShowType = route.params.type
			console.log('route.params', route.params.type)
			;(async () => {
				await previewFns
					.details({ id: route.params.id })
					.then((res) => {
						data.parametersData.title = res.data.title
						data.parametersData.imageUrl = res.data.imageUrl
						data.parametersData.excerpt = res.data.excerpt
						data.parametersData.content = res.data.content
						data.leftInfo = res.data.title
						data.infoImageUrl = res.data.imageUrl
						data.returnUrl = res.data.returnUrl
						if (route.params.type == 'img') {
							data.contentMusic = res.data.mainVideoUrl
							data.parametersData.imagePage = res.data.imagePage
							// if(res.data.imagePage.length>0){
